...
package com.iclicardeche.softkeyboard;
...
import com.iclicardeche.dataoperation.AppLauncher;
import com.iclicardeche.dataoperation.JSONParser;
...
public class SoftKeyboard extends InputMethodService implements
	...
	private boolean translateKeyDown(int keyCode, KeyEvent event) {
		...
		Log.v("keyboardListener", "tagis=" + tagId);

		if (event.getKeyCode() == KeyEvent.KEYCODE_ENTER) {
			// TODO comparison with table to launch action
			try {
				if (compareFullToList(tagId) == true) {
					Log.v("keyboardListener", "tagisintrkeydown=" + tagId
							+ "-enter");
					return true;
				}
			} catch (Exception e) {
		 ...
		 * tagId += (char)event.getUnicodeChar(); else
		 */
		onKey(c, null);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		case KeyEvent.KEYCODE_ENTER:
			try {
				if (compareFullToList(tagId) == true) {
					Log.v("keyboardListener", "TOTOTOkeydown=" + tagId
							+ "-enter");
					tagId = "";
					return true;
				} else {
		...
		default:
			...
			if (PROCESS_HARD_KEYS) {
				 ...
				 * keyCode, event); int c =
				 * event.getUnicodeChar(MetaKeyKeyListener
				 * .getMetaState(mMetaState));
				 */
				try {
					if (compareStartToList(tagId, (char) event.getUnicodeChar()) == true) {
